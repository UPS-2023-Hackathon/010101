...
def is_inside_geofence(building, latitude, longitude):
    if building in geofence_data:
        geofence_points = geofence_data[building]
        # Check if the given latitude and longitude are inside the geofence polygon
        n = len(geofence_points)
        inside = False
        p1x, p1y = geofence_points[0]
        for i in range(n+1):
            p2x, p2y = geofence_points[i % n]
...
latitude = 38.1305
longitude = -85.719
inside = is_inside_geofence(building, latitude, longitude)

if inside:
    print("Equipment is inside the geofence.")
else:
    print("Equipment is outside the geofence.")
